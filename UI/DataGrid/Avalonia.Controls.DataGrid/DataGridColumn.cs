        ...
        public bool CanUserSort
        {
            get
            {
                ...
                else if (OwningGrid != null)
                {
                    ...
                    var itemType = OwningGrid.DataConnection.DataType;
                    return !string.IsNullOrEmpty(propertyPath)
                        && itemType is not null
                        && DataGridSortAccessorRegistry.TryCreate(itemType, propertyPath, OwningGrid.DataConnection.CollectionView?.Culture, out _);
                }
                else
                {
