...
namespace Avalonia.Collections
{
    ...
    abstract class DataGridSortDescription
    {
        ...
        {
            return new DataGridRegisteredPathSortDescription(propertyPath, direction, culture);
        }

        public static DataGridSortDescription FromPath(string propertyPath, ListSortDirection direction, IComparer comparer)
        {
            return new DataGridRegisteredPathSortDescription(propertyPath, direction, null);
        }
    ...
    internal sealed class DataGridRegisteredPathSortDescription : DataGridSortDescription
    {
        private readonly string _propertyPath;
        private readonly ListSortDirection _direction;
        private readonly CultureInfo _culture;
        private RegisteredSortAccessor _accessor;
        private bool _resolved;

        public DataGridRegisteredPathSortDescription(string propertyPath, ListSortDirection direction, CultureInfo culture)
        {
            _propertyPath = propertyPath;
            _direction = direction;
            _culture = culture;
        ...
        public override string PropertyPath => _propertyPath;
        public override ListSortDirection Direction => _direction;
        public override IComparer<object> Comparer => _accessor.Comparer;

        public override DataGridSortDescription SwitchSortDirection()
        {
            var nextDirection = _direction == ListSortDirection.Ascending ? ListSortDirection.Descending : ListSortDirection.Ascending;
            return new DataGridRegisteredPathSortDescription(_propertyPath, nextDirection, _culture);
        }
        ...
        internal override void Initialize(Type itemType)
        {
            ...
            _resolved = true;
            DataGridSortAccessorRegistry.TryCreate(itemType, _propertyPath, _culture, out _accessor);
        }

        public override IOrderedEnumerable<object> OrderBy(IEnumerable<object> seq)
        {
            return _direction == ListSortDirection.Descending
                ? seq.OrderByDescending(_accessor.GetValue, _accessor.Comparer)
                : seq.OrderBy(_accessor.GetValue, _accessor.Comparer);
        }

        public override IOrderedEnumerable<object> ThenBy(IOrderedEnumerable<object> seq)
        {
            return _direction == ListSortDirection.Descending
                ? seq.ThenByDescending(_accessor.GetValue, _accessor.Comparer)
                : seq.ThenBy(_accessor.GetValue, _accessor.Comparer);
        }
    }
}
