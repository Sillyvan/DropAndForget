...
public sealed class DialogManager
{
    ...
    internal readonly Dictionary<Type, Func<Control>> CustomDialogs = [];
    ...
    /// <returns></returns>
    public DialogManager Register<TView, TContext>() where TView : Control, new()
    {
        CustomDialogs[typeof(TContext)] = static () => new TView();
        return this;
    }
